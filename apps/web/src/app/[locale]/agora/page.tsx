...
export default async function AgoraPage(props: { params: Promise<{ locale: string }>; searchParams: Promise<SearchParams> }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 space-y-6 sm:space-y-8">
      ...
      <header className="animate-fade-up">
        <h1 className="text-3xl font-semibold tracking-tight text-[var(--text-primary)]">{t("Agora")}</h1>
        <p className="text-sm text-[var(--text-muted)] mt-1">
          {t("Discover pay-per-use APIs and services on the XRPL x402 network.")}
          {totalCount > 0 && <span className="ml-2 text-[var(--text-muted)]">{t("({n} resources)").replace("{n}", String(totalCount))}</span>}
        </p>
      </header>
      ...
      {resources.length === 0 ? (
        <div className="dashboard-panel bg-[var(--bg-surface)] border border-[var(--border)] p-10 sm:p-16 text-center animate-fade-up" style={{ animationDelay: "80ms" }}>
          <h2 className="text-lg font-medium text-[var(--paper)] mb-2">{t("No resources listed yet")}</h2>
          <p className="text-sm text-[var(--text-muted)] mb-6">{t("Be the first to register an x402-compatible API on the XRPL.")}</p>
                ...
                <div className="ui-card bg-[var(--bg-surface)] rounded-xl border border-[var(--border)] p-4 sm:p-6 h-full hover:border-[rgba(0,140,255,0.28)] transition-all flex flex-col">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <span className={`shrink-0 px-2 py-0.5 rounded text-[10px] uppercase tracking-wider font-semibold ${
                      ...
                      {res.isDiscovered ? t("Discovered") : t("Registered")}
                    </span>
                  </div>
                  ...
                  <div className="mt-auto flex flex-wrap items-center justify-between gap-2 pt-4 border-t border-[var(--border)]">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                      <span className="text-[10px] text-[var(--text-muted)]">
                        {t("{n} txs").replace("{n}", String(res._count?.transactions ?? 0))}
                      </span>
