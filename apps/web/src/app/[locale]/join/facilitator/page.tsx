...
export default function JoinFacilitatorPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResult(res.ok && data.success ? { ok: true } : { error: data.error || t("Failed to register.") });
    } catch {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
      ...
      <header className="mb-10 animate-fade-up">
        <span className="text-[10px] font-plek uppercase tracking-[0.28em] text-[var(--paper-mute)]">{t("Join · Facilitator")}</span>
        <h1 className="mt-5 text-4xl sm:text-5xl font-medium tracking-[-0.03em] leading-[1.02] text-[var(--paper)]">{t("Register your")}<br />{t("facilitator")}</h1>
        <p className="text-[15px] text-[var(--text-secondary)] mt-5 max-w-lg leading-relaxed">
          {t("Submit your facilitator and its SourceTag. We index every XRPL Payment carrying that tag, so your settled volume appears on the Index. No ownership proof required.")}
        </p>
      </header>
      ...
      <form onSubmit={submit} className="dashboard-panel bg-[var(--bg-surface)] border border-[var(--border)] p-6 sm:p-8 space-y-5 animate-fade-up" style={{ animationDelay: "80ms" }}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            <label htmlFor="fac-name" className={label}>{t("Facilitator name")}</label>
            <input id="fac-name" className={input} required value={form.name} onChange={set("name")} placeholder={t("Acme Facilitator")} />
          </div>
          <div>
        ...
        <div>
          <label htmlFor="fac-website" className={label}>{t("Website")}</label>
          <input id="fac-website" className={input} value={form.website} onChange={set("website")} placeholder="https://facilitator.example" />
        </div>
        <div>
          <label htmlFor="fac-url" className={label}>{t("Facilitator URL")}</label>
          <input id="fac-url" className={input} value={form.url} onChange={set("url")} placeholder="https://facilitator.example/api" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            <label htmlFor="fac-networks" className={label}>{t("Networks (comma-separated)")}</label>
            <input id="fac-networks" className={input} value={form.networks} onChange={set("networks")} placeholder="xrpl:0, xrpl:1" />
          </div>
          <div>
            <label htmlFor="fac-assets" className={label}>{t("Assets (comma-separated)")}</label>
            <input id="fac-assets" className={input} value={form.assets} onChange={set("assets")} placeholder="XRP, RLUSD" />
          </div>
        </div>
        <div>
          <label htmlFor="fac-description" className={label}>{t("Description")}</label>
          <textarea id="fac-description" className={input} rows={3} value={form.description} onChange={set("description")} placeholder={t("What your facilitator does.")} />
        </div>
        ...
        <button type="submit" disabled={loading} className="ui-control w-full bg-[var(--brand-blue)] text-white font-medium py-3 px-4 rounded-lg transition-all disabled:opacity-50">
          {loading ? t("Registering…") : t("Register facilitator")}
        </button>

        {result?.ok ? (
          <div className="p-4 rounded-lg text-sm border bg-[var(--success)]/10 border-[var(--success)]/20 text-[var(--success)]">
            {t("Registered. Your SourceTag is now indexed — settled payments will appear on the Index.")}
          </div>
        ) : null}
        {result?.error ? (
