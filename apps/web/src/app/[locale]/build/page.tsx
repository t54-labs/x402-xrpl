...
const META_EN = {
  title: "Build",
  description:
    "Why and how to build x402 agentic payments on XRPL: gate a route, pay as an agent, and add Verifiable Intent — with the t54 facilitator.",
};
...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: locale === "en" ? "/build" : `/${locale}/build`,
      languages: { en: "/build", ko: "/ko/build" },
    },
  };
}
...
const STRINGS_EN = {
  ...
  part03: "Build it",
  part04: "Reference",
  why_h2: "What general-purpose chains can’t give an agent — built into the protocol",
  why_p:
  ...
  how_h2: "One request, one signed Payment, one settlement",
  how_p: "x402 is an HTTP-native payment handshake. Four steps, every one of them inspectable on the wire and on-ledger.",
  build_h2: "From zero to a paid endpoint — both sides of the call",
  ref_h2: "Endpoints, assets, and where to go deeper",
  console_caption: "Server gates the route · Agent pays the 402 · Verifiable Intent binds who may spend and how much. Toggle TS / PY.",
  panel_facilitator_title: "Hosted facilitator",
  panel_assets_title: "Assets & amounts",
  mainnet_label: "Mainnet · xrpl:0",
  testnet_label: "Testnet · xrpl:1",
  endpoints_label: "Endpoints",
  xrp_label: "XRP",
  xrp_value: "priced in drops (1e6 = 1 XRP)",
  iou_label: "RLUSD / IOU",
  iou_value: "decimal value + issuer",
  networks_label: "Networks",
  cta_list: "List your endpoint",
  link_faq: "Engineering FAQ",
  link_resources: "Browse all resources",
  link_why: "The full case for XRPL",
};

export default async function BuildPage(props: Props) {
  const locale = await resolveLocale(props);
  const isKo = locale === "ko";
  const t = isKo ? { ...STRINGS_EN, ...BUILD_KO.strings } : STRINGS_EN;
  const why = isKo ? BUILD_KO.why : WHY;
  const flow = isKo ? BUILD_KO.flow : FLOW;
  const steps = isKo ? BUILD_KO.steps : STEPS;
  const refs = isKo ? BUILD_KO.refs : REFS;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-8 py-16 sm:py-24">
      <header className="animate-fade-up max-w-3xl">
        <span className="text-[10px] font-plek uppercase tracking-[0.28em] text-[var(--paper-mute)]">{t.kicker}</span>
        ...
        <p className="mt-6 text-lg text-[var(--text-secondary)] leading-relaxed max-w-2xl">{t.intro}</p>
      </header>

      {/* 01 — Why XRPL */}
      <section className="mt-20 animate-fade-up" style={{ animationDelay: "60ms" }}>
        <PartLabel n="01" total="04" title={t.part01} />
        <h2 className="mt-7 text-2xl sm:text-3xl font-medium tracking-tight text-[var(--paper)] max-w-2xl">{t.why_h2}</h2>
        <p className="mt-4 text-[15px] text-[var(--text-secondary)] leading-relaxed max-w-2xl">{t.why_p}</p>
        <div className="mt-10 space-y-px">
          {why.map((w) => (
            <div key={w.spec} className="grid grid-cols-1 sm:grid-cols-[210px_1fr] gap-1.5 sm:gap-8 py-6 border-t border-[var(--rule)]">
      ...
      {/* 02 — How it works */}
      <section className="mt-20 animate-fade-up" style={{ animationDelay: "60ms" }}>
        <PartLabel n="02" total="04" title={t.part02} />
        <h2 className="mt-7 text-2xl sm:text-3xl font-medium tracking-tight text-[var(--paper)] max-w-2xl">{t.how_h2}</h2>
        <p className="mt-4 text-[15px] text-[var(--text-secondary)] leading-relaxed max-w-2xl">{t.how_p}</p>
      ...
      {/* 03 — Build it */}
      <section className="mt-20 animate-fade-up" style={{ animationDelay: "60ms" }}>
        <PartLabel n="03" total="04" title={t.part03} />
        <h2 className="mt-7 text-2xl sm:text-3xl font-medium tracking-tight text-[var(--paper)] max-w-2xl">{t.build_h2}</h2>
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] gap-10 items-start">
          ...
          <div className="lg:sticky lg:top-24">
            <BuildConsole />
            <p className="mt-3 text-[12px] text-[var(--text-muted)] leading-relaxed">{t.console_caption}</p>
          </div>
        </div>
      </section>
      ...
      {/* 04 — Reference */}
      <section className="mt-20 animate-fade-up" style={{ animationDelay: "60ms" }}>
        <PartLabel n="04" total="04" title={t.part04} />
        <h2 className="mt-7 text-2xl sm:text-3xl font-medium tracking-tight text-[var(--paper)] max-w-2xl">{t.ref_h2}</h2>

        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="dashboard-panel bg-[var(--bg-surface)] border border-[var(--border)] p-6">
            <h3 className="text-[11px] font-plek uppercase tracking-[0.18em] text-[var(--paper-mute)]">{t.panel_facilitator_title}</h3>
            <dl className="mt-4 space-y-3 text-[13px]">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4">
                <dt className="text-[var(--text-muted)] shrink-0">{t.mainnet_label}</dt>
                <dd className="font-mono text-[12px] text-[var(--text-secondary)] break-all">xrpl-facilitator-mainnet.t54.ai</dd>
              </div>
              <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4">
                <dt className="text-[var(--text-muted)] shrink-0">{t.testnet_label}</dt>
              ...
              <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4 pt-2 border-t border-[var(--rule)]">
                <dt className="text-[var(--text-muted)] shrink-0">{t.endpoints_label}</dt>
          ...
          <div className="dashboard-panel bg-[var(--bg-surface)] border border-[var(--border)] p-6">
            <h3 className="text-[11px] font-plek uppercase tracking-[0.18em] text-[var(--paper-mute)]">{t.panel_assets_title}</h3>
            <dl className="mt-4 space-y-3 text-[13px]">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4">
                <dt className="text-[var(--text-muted)] shrink-0">{t.xrp_label}</dt>
                <dd className="text-[var(--text-secondary)] sm:text-right">{t.xrp_value}</dd>
              </div>
              <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4">
                <dt className="text-[var(--text-muted)] shrink-0">{t.iou_label}</dt>
                <dd className="text-[var(--text-secondary)] sm:text-right">{t.iou_value}</dd>
              </div>
              <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4 pt-2 border-t border-[var(--rule)]">
                <dt className="text-[var(--text-muted)] shrink-0">{t.networks_label}</dt>
        ...
        <div className="mt-10 flex flex-wrap items-center gap-x-6 gap-y-2">
          <Link href="/join/service" className="ui-control px-4 py-2 bg-[var(--brand-blue)] text-white font-medium text-sm">{t.cta_list}</Link>
          <Link href="/faq" className="text-sm text-[var(--text-secondary)] hover:text-[var(--paper)] transition-colors">{t.link_faq} &rarr;</Link>
          <Link href="/resources" className="text-sm text-[var(--text-secondary)] hover:text-[var(--paper)] transition-colors">{t.link_resources} &rarr;</Link>
          <Link href="/why-xrpl" className="text-sm text-[var(--text-secondary)] hover:text-[var(--paper)] transition-colors">{t.link_why} &rarr;</Link>
        </div>
      </section>
    </div>
  );
