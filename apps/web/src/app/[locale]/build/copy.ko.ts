// Korean copy for the Build page — generated from the translation pass.
export const BUILD_KO = {
  "why": [
    {
      "spec": "Native DEX · auto-bridging",
      "title": "Payment 하나, 어떤 자산이든",
      "body": "AI 에이전트는 XRP로 결제하고, 가맹점은 RLUSD(미국 달러에 1:1로 연동된 디지털 화폐)를 받습니다. 이 교환은 XRPL에 처음부터 내장된 거래소(DEX)를 거쳐, 전부 성공하거나 전부 취소되는 방식으로 처리됩니다. 별도의 환전 절차도, 중간 경유 시스템도, 중간에 멈춘 상태도 없습니다. 범용 체인에서는 승인(approve) → 환전(swap) → 지불(pay)을 각각 따로 수행해야 하고, 각 단계가 독립적으로 실패할 수 있습니다."
    },
    {
      "spec": "XLS-70 / XLS-80",
      "title": "Know-Your-Agent, 프로토콜 안에서",
      "body": "원장에 기록되는 Credentials와 Permissioned Domains를 사용하면, 거래 상대방은 발행 기관이 서명한 증명(신원 확인(KYC) 완료, 제재 대상 아님)을 원장 위에서 직접 검증할 수 있습니다. KYA(Know-Your-Agent)는 따로 붙이는 중간 프로그램(미들웨어)이 아니라, 정산 시점에 원장에서 강제됩니다. 증명 뒤에 있는 개인정보(PII)는 전혀 노출되지 않습니다."
    },
    {
      "spec": "DepositAuth · Freeze / Clawback",
      "title": "허용 목록과 발행자 구제 수단",
      "body": "받는 쪽은 크리덴셜(자격 증명)을 가진 보내는 쪽만 받도록 제한할 수 있습니다(DepositAuth / DepositPreauth). 규제 대상 토큰의 발행자는 분쟁 중이거나 탈취된 잔액을 Freeze로 동결하거나 Clawback으로 회수할 수 있습니다(XLS-39). 자산을 대신 보관해 주는 제3자 없이 구제 수단을 확보하면서도, 기본 레이어의 XRP 결제는 최종성(한 번 확정되면 되돌릴 수 없음)을 유지합니다."
    },
    {
      "spec": "Escrow · Payment Channels",
      "title": "프로그래머블 정산",
      "body": "Escrow에 대금을 잠가 두고, 증명이 제출되거나 기한이 되면 지급되게 할 수 있습니다(인도 시 지급). 또는 호출 한 건 단위의 소액 결제를 Payment Channel로 이어 보낸 뒤, 원장 위에서 한 번에 정산할 수 있습니다. 조건이 붙거나 아주 잦은 기계 간 결제를, 직접 만들고 일일이 점검해야 하는 별도의 계약 프로그램(스마트 컨트랙트) 없이 구현합니다."
    },
    {
      "spec": "Mastercard Verifiable Intent",
      "title": "자금이 움직이기 전에, 권한이 증명됩니다",
      "body": "x402 Secure는 Verifiable Intent 체인을 구현합니다. 이 체인은 Know-Your-Agent 크리덴셜, 지출 한도가 담긴 소유자 서명 위임, 결제 건별 AI 에이전트 서명으로 구성됩니다. 정산 전에 리스크 정책과 대조해 검증됩니다. Verifiable Intent는 Mastercard의 프레임워크(Agent Pay for Machines)이며, Ripple과 t54 Labs가 공식 파트너로 명시되어 있습니다."
    }
  ],
  "flow": [
    {
  ...
  "strings": {
    ...
    "h1": "AI 에이전트 경제를\nXRPL 위에 구축하세요",
    "intro": "x402는 HTTP 402를 실제로 작동하게 만듭니다. 서버가 요청에 가격으로 응답하면, AI 에이전트(사람 대신 자동으로 일하는 인공지능 프로그램)가 XRPL Payment에 서명합니다. t54 facilitator(결제 중개 서비스)가 이를 검증해 몇 초 안에 원장(모든 거래가 기록되는 장부)에서 정산합니다. 자산을 맡길 필요도, API 키도 없습니다. XRPL이 왜 머신 결제에 알맞은 기반인지, 그리고 첫 유료 엔드포인트(접속 주소)를 어떻게 출시하는지 소개합니다.",
    "part01": "왜 XRPL인가",
    "part02": "작동 방식",
    "part03": "구축하기",
    "part04": "레퍼런스",
    "why_h2": "범용 체인이 AI 에이전트에게 주지 못하는 것 — XRPL 프로토콜에는 내장되어 있습니다",
    "why_p": "“빠르고 저렴하다”는 기본 전제일 뿐입니다. AI 에이전트 상거래에서 XRPL의 강점은 기관 친화적이고 결제에 특화되어 있다는 점입니다. 신원 확인, 구제 수단, 조건부 정산이 따로 신뢰해야 하는 계약 프로그램(스마트 컨트랙트)이 아니라 원장 자체에 들어 있습니다.",
    "how_h2": "요청 하나, 서명된 Payment 하나, 정산 하나",
    "how_p": "x402는 HTTP 위에서 그대로 동작하는 결제 핸드셰이크(정해진 순서로 요청과 응답을 주고받는 절차)입니다. 네 단계로 이루어지며, 모든 단계를 주고받는 내용과 원장에서 직접 확인할 수 있습니다.",
    "build_h2": "처음부터 유료 엔드포인트까지 — 호출하는 쪽과 받는 쪽 모두",
    "ref_h2": "엔드포인트, 자산, 그리고 더 깊이 볼 자료",
    "console_caption": "서버는 라우트에 결제 관문을 세우고 · AI 에이전트는 402를 결제하고 · Verifiable Intent는 누가 얼마까지 쓸 수 있는지 묶어 둡니다. TS / PY로 전환할 수 있습니다.",
